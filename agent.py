...
def extract_suggestion(comment: str, student_speech: str) -> str:
    ...
    Strips trailing explanations like " - it sounds more natural" or "because..."
    """
    import re

    # Try to find quoted text first — most reliable
    quoted = re.search(r'["\']([^"\']{10,})["\']\s*(?:-|because|as|which|since|\.|$)', comment)
    if quoted:
        return quoted.group(1).strip()

    # Try phrase-based patterns
    phrase_patterns = [
        r'(?:[Yy]ou could say|[Tt]ry saying|[Aa]nother way to put it is|[Ii]t would sound more natural to say|[Aa] lot of people would say|[Yy]ou might also hear people say|[Aa] more natural way to say this is)[,:]?\s*["\']?([^"\'.]{10,?})["\']?(?:\s*[-–]|\s*because|\s*as|\s*which|\s*since|\.|$)',
    ]
